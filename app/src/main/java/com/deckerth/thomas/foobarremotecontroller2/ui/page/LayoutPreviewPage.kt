...
@Composable
fun LayoutPreviewPage(appViewModel: AppViewModel, layoutViewModel: LayoutViewModel) {
    ...
    if (appViewModel.selectedView == ViewsWithLayout.PLAYER) {
        BoxWithConstraints(
            ...
            PlayerCard(
                appViewModel,
                layoutViewModel.currentPlayer,
                true,
                { layoutViewModel.currentPlayer = layoutViewModel.previewPlayerPop },
                { layoutViewModel.currentPlayer = layoutViewModel.previewPlayerClassic },
                boxSize = IntSize(maxBoxWidth.value.toInt(), maxBoxHeight.value.toInt())
            )
        }
    } else {
        setupPreviewPlaylist()
        PlaylistPreview(appViewModel, layoutViewModel, previewPlaylist)
    }
}

@Composable
fun PlaylistPreview(vm: AppViewModel, layoutViewModel: LayoutViewModel, playlist: Playlist) {
    val playlistState =
    ...
    LazyColumn(state = playlistState) {
        try {
            items(playlist.albums) { album ->
                AlbumCard(vm, layoutViewModel.currentPlayer, album, layoutManager.getLayout(Layouts.LAYOUT_CUSTOM), true)
            }
        } catch (e: Exception) {
            e.printStackTrace()
        }
